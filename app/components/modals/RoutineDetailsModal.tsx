              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (level) {
                      setEditedRoutine(prev => 
                        prev ? { ...prev, level, color: level.color } : prev
                      );
                    } else {
                      setEditedRoutine(prev => 
                        prev ? { ...prev, level: undefined, color: '#3b82f6' } : prev
                      );
                    }
                  }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white text-gray-900"
      ...
      <ManageGenresModal
        ...
        onChange={(next) => {
          ...
          if (!next.find(g => g.id === editedRoutine.genre.id) && next.length > 0) {
            ...
            setEditedRoutine(prev => (prev ? { ...prev, genre: newGenre } : prev));
          }
        }}
      />
      ...
      <ManageLevelsModal
        ...
        onChange={(next) => {
          ...
          if (editedRoutine.level && !updatedLevels.find(l => l.id === editedRoutine.level?.id)) {
            setEditedRoutine(prev => (prev ? { ...prev, level: undefined, color: '#3b82f6' } : prev));
          } else if (editedRoutine.level && updatedLevels.find(l => l.id === editedRoutine.level?.id)) {
            // Update color if level still exists but color may have changed
            const updatedLevel = updatedLevels.find(l => l.id === editedRoutine.level?.id);
            if (updatedLevel) {
              setEditedRoutine(prev => (prev ? { ...prev, level: updatedLevel as unknown as Level, color: (updatedLevel as unknown as Level).color } : prev));
            }
          }
        }}
      />
